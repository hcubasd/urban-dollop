...
def read_network(path):
    """(geometries, attributes_complete) if `path` exists, else None --
    None signals full synthesis. Raises ValueError if the file exists but
    doesn't meet the leaf contract: 'grade'/'road_type'/'oneway' present
    and -- checked jointly, since they're synthesized together --
    entirely empty or entirely populated (never a mix), and geometry
    entirely populated (there's no legitimate reason for topology to be
    partially given, same reasoning as zones.gpkg). link_id is not part
    of this contract at all -- it's a plain positional index, not a real
    identifier, so it's always reassigned fresh regardless of what's
    given, the same way agent_id is never something a caller supplies.
    """
    ...
    return gdf.geometry.tolist(), all_filled
