...
def test_one_link_one_pollutant(emission_factors, loaded_links_simple):
    result = calculate_emissions(loaded_links_simple, emission_factors, cfg())
    assert len(result) == 1
    r = result[0]
    assert r.link_id == 1
...
def test_output_fields_populated(emission_factors, loaded_links_simple):
    result = calculate_emissions(loaded_links_simple, emission_factors, cfg())
    r = result[0]
    assert r.n_trips == 1
    assert r.distance_m == 1000.0
    assert r.grade_pct == 0.0
...
def test_empty_factors_returns_empty(loaded_links_simple):
    assert calculate_emissions(loaded_links_simple, [], cfg()) == []


# ── emission quantity ────────────────────────────────────────────────────────


def test_emission_at_zero_grade_zero_load(emission_factors, loaded_links_simple):
    # grade=0, fill_rate=0 → load=0 → EF=100 g/km; 1 trip × 1 km = 100 g
    result = calculate_emissions(loaded_links_simple, emission_factors, cfg(fill_rate=0.0))
    assert result[0].emission_g == pytest.approx(100.0)


def test_emission_at_full_load(emission_factors, loaded_links_simple):
    # grade=0, fill_rate=1 → load=100 → EF=200 g/km; 1 km = 200 g
    result = calculate_emissions(loaded_links_simple, emission_factors, cfg(fill_rate=1.0))
    assert result[0].emission_g == pytest.approx(200.0)


def test_emission_at_half_load_interpolated(emission_factors, loaded_links_simple):
    # grade=0, fill_rate=0.5 → load=50 → EF interpolated between 100 and 200 = 150
    result = calculate_emissions(loaded_links_simple, emission_factors, cfg(fill_rate=0.5))
...
def test_emission_grade_interpolation(emission_factors):
    ...
    link = LoadedLink(
        ...
        grade_pct=1.0, vehicle_id=1, n_trips=1,
    )
    result = calculate_emissions([link], emission_factors, cfg(fill_rate=0.0))
...
def test_emission_bilinear_interpolation(emission_factors):
    ...
    result = calculate_emissions([link], emission_factors, cfg(fill_rate=0.5))
...
def test_n_trips_scales_emission(emission_factors):
    link = LoadedLink(
        link_id=1, road_type="urban", distance_m=1000.0,
        grade_pct=0.0, vehicle_id=1, n_trips=5,
    )
    result = calculate_emissions([link], emission_factors, cfg(fill_rate=0.0))
...
def test_distance_scales_emission(emission_factors):
    link = LoadedLink(
        link_id=1, road_type="urban", distance_m=2000.0,
...
def test_grade_above_max_is_clamped(emission_factors):
    ...
    link = LoadedLink(
        link_id=1, road_type="urban", distance_m=1000.0,
        grade_pct=10.0, vehicle_id=1, n_trips=1,
    )
    result = calculate_emissions([link], emission_factors, cfg(fill_rate=0.0))
    assert result[0].emission_g == pytest.approx(150.0)
...
def test_grade_below_min_is_clamped(emission_factors):
    ...
    link = LoadedLink(
        link_id=1, road_type="urban", distance_m=1000.0,
        grade_pct=-5.0, vehicle_id=1, n_trips=1,
    )
    result = calculate_emissions([link], emission_factors, cfg(fill_rate=0.0))
    assert result[0].emission_g == pytest.approx(100.0)
...
def test_multiple_links_produce_one_row_each(emission_factors):
    links = [
        LoadedLink(link_id=1, road_type="urban", distance_m=1000.0,
                   grade_pct=0.0, vehicle_id=1, n_trips=2),
        LoadedLink(link_id=2, road_type="urban", distance_m=500.0,
                   grade_pct=0.0, vehicle_id=1, n_trips=1),
    ]
    result = calculate_emissions(links, emission_factors, cfg())
    assert len(result) == 2
    assert {r.link_id for r in result} == {1, 2}


def test_multiple_pollutants(loaded_links_simple):
    nox_factors = [
        EmissionFactor(
    ...
    result = calculate_emissions(
        loaded_links_simple, co2_factors + nox_factors, cfg()
    )
    assert len(result) == 2
...
def test_hour_passed_through(emission_factors):
    link = LoadedLink(
        link_id=1, road_type="urban", distance_m=1000.0,
        grade_pct=0.0, vehicle_id=1, n_trips=3, hour=8,
    )
    result = calculate_emissions([link], emission_factors, cfg())
...
def test_rf_reduces_emission():
    ...
    result = calculate_emissions([link], factors, cfg(fill_rate=0.0))
...
def test_speed_affects_polynomial_emission():
    ...
    result = calculate_emissions([link], factors, cfg(speed_urban=10.0))
    assert result[0].emission_g == pytest.approx(100.0)

    # Speed = 20 km/h → EF = 400 g/km; 1 km → 400 g
    result2 = calculate_emissions([link], factors, cfg(speed_urban=20.0))
...
def test_missing_vehicle_factors_raises(loaded_links_simple):
    factors = [
        EmissionFactor(
    ...
    with pytest.raises(ValueError, match="Vehicle IDs"):
        calculate_emissions(loaded_links_simple, factors, cfg())


def test_missing_road_type_speed_raises(emission_factors):
    link = LoadedLink(
        link_id=1, road_type="highway", distance_m=1000.0,
        grade_pct=0.0, vehicle_id=1, n_trips=1,
    )
    with pytest.raises(ValueError, match="speed"):
        calculate_emissions([link], emission_factors, cfg())
...
def test_incomplete_factor_grid_raises():
    ...
    with pytest.raises(ValueError, match="Incomplete emission factor grid"):
        calculate_emissions([link], factors, cfg())


def test_config_required_raises(emission_factors, loaded_links_simple):
    with pytest.raises(ValueError, match="EmissionCalculationConfig"):
        calculate_emissions(loaded_links_simple, emission_factors, config=None)
