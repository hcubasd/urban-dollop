...
def test_total_parcels_preserved(
    zones, depots, carriers, vehicles, skim, demand_config
):
    demands = generate_parcel_demand(zones, depots, carriers, skim, demand_config)
    trips = schedule_parcel_deliveries(demands, vehicles, skim)
...
def test_trip_ids_are_sequential_per_tour(
    ...
    trips = schedule_parcel_deliveries(
        demands, vehicles, skim, ParcelSchedulingConfig(seed=0)
    )
...
def test_raises_for_origin_zone_not_in_skim(vehicles, skim):
    bad_demands = [
    ...
    with pytest.raises(ValueError, match="not present in the skim"):
        schedule_parcel_deliveries(bad_demands, vehicles, skim)


def test_empty_demands_produces_no_trips(vehicles, skim):
    trips = schedule_parcel_deliveries([], vehicles, skim)
    assert trips == []
